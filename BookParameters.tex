...
\usepackage[scaled=0.95]{helvet}% High-quality sans-serif font
...
\pgfmathsetmacro\TotalPageCount{530}% Must be manually entered
\pgfmathsetmacro\PaperWidthPt{6.14in}%
\pgfmathsetmacro\PaperHeightPt{9.21in}%

\newlength{\PaperW}\setlength{\PaperW}{6.14in}
\newlength{\PaperH}\setlength{\PaperH}{9.21in}
\newlength{\HorBleed}\setlength{\HorBleed}{0.125in}
\newlength{\VerBleed}\setlength{\VerBleed}{0.125in}

%\def\ShowCoverStatistics{}% Enable to show some numbers on back cover.

\def\TheMainTitle{Complete C++ Low Latency}
\def\TheSubTitle{}
\def\TheSubTitleA{}
\def\TheSubTitleB{}
\def\TheBookSeries{}

\def\TheAuthor{Jose Ignacio Seco Sanz}
\def\TheAuthorA{Wilson J.}
\def\TheAuthorB{Schmeggles}
\def\TheAuthorLNF{Schmeggles, Wilson, J.}

\def\TheCopyrightYear{2025}
...
\textsf{\textbf{{Complete C++ Low Latency} -- Master the techniques that separate ordinary C++ code from truly high-performance systems.}}

\textsf{\textbf{Low-latency programming is about far more than writing fast code. It is about understanding how modern CPUs, memory hierarchies, operating systems, compilers, and concurrency interact to determine the performance of your applications. Whether you're building trading systems, game engines, real-time analytics, networking software, or any application where every microsecond matters, these principles are essential.}}

\textsf{\textbf{Complete C++ Low Latency is a practical, in-depth guide to designing and implementing software that minimizes latency while maximizing throughput and predictability. Instead of relying on myths and premature optimizations, you'll learn the engineering principles used to build modern high-performance C++ applications.}}

\textsf{\textbf{The book covers both fundamental concepts and advanced optimization techniques, always explaining why an optimization works, when it should be applied, and when it should be avoided.}}

\textsf{\textbf{Rather than presenting isolated tricks, Complete C++ Low Latency develops a systematic understanding of performance engineering, enabling you to make informed design decisions in your own projects.}}
}

\def\AuthorBio{% 
\textsf{\textbf{Jose Ignacio Seco Sanz}}}
